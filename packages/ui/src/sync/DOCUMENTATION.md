...
Codex projections remain explicitly engine-tagged and never enter OpenCode event normalization. The pipeline validates raw Codex envelopes as a separate branch and routes malformed envelopes, transport backpressure, and reconnects to authoritative repair instead of applying partial state. The session message loader subscribes before fetching the projected snapshot, buffers at most 256 events per session, atomically replaces messages, parts, status, approvals, and diff, then applies only strictly newer contiguous revisions. Gaps and overflow fail closed to at most two consecutive repair reads per trigger; failed or non-converging reads preserve the last valid state and expose an error. Runtime changes invalidate stale completions and temporary buffers, while unaffected session references remain stable.
Codex approval cards route only `once` or `reject` through the authenticated Codex client; OpenCode `always` and auto-accept paths remain isolated. The server owns atomic reply claims and authoritative pending-approval re-projection, so UI disabling is not a correctness boundary.
Codex session IDs route abort through the authenticated Codex endpoint, while OpenCode sessions retain the existing directory-scoped SDK abort path. Codex abort failures propagate because only the server can authoritatively prove interrupt and cleanup; the UI never converts them to idle success.
Codex prompt routing uses an explicit execution-target discriminant. Browser Web drafts may select Codex only after the authenticated server capability confirms availability; Electron, VS Code, hosted-mobile, and Capacitor surfaces keep Codex unavailable. Codex session creation and prompts use the authenticated Codex client without OpenCode provider/model fields, while OpenCode keeps its existing SDK, optimistic, provider/model, agent, variant, queue, and command behavior. Execution-target selection is in-memory, reset on runtime change, and corrected from the selected session identity so stale draft or session choices cannot reroute a prompt. Snapshot/bootstrap/reconnect reconciliation remains separate lifecycle work.
