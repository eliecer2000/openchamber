...
Codex projections remain explicitly engine-tagged and never enter OpenCode event normalization. The pipeline validates, revision-orders, and deduplicates only the incoming Codex batch; the Codex reducer rejects revisions at or below the caller's authoritative revision and clones each affected message or part bucket at most once across the accepted batch. Malformed or unknown projections are isolated, so they cannot clear or block unrelated complete session state. Transport and snapshot/reconnect wiring remain separate lifecycle work.
